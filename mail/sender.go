...
func NewGmailSender(name string, fromEmailAddress string, fromEmailPassword string) EmailSender {
	return &GmailSender{
		name:              name,
		fromEmailAddrres:  fromEmailAddress,
...
func (sender *GmailSender) SendEmail(subject string, content string, to []string, cc []string, bcc []string, attachFiles []string) error {
	e := email.NewEmail()
	e.From = fmt.Sprintf("%s <%s>", sender.name, sender.fromEmailAddrres)
	e.Subject = subject
	e.HTML = []byte(content)
	e.To = to
	e.Cc = cc
	...
	smtpAuth := smtp.PlainAuth("", sender.fromEmailAddrres, sender.fromEmailPassword, smtpAuthAddres)
